function func = lognewfractionTe(a)
...
switch a
    case 1; func = @lognewfractionTeGen;
    case 2; func = @lognewfractionTeApply;
    case 3; func = @lognewfractionTeApplyColo;
    case 4; func = @lognewfractionTeApplyField;
    case 5; func = @lognewfractionTeReport;
end
...
%--------------------------------------------------------------------------
function mdlstruct = lognewfractionTeGen(Y,X,settingsSet)
...
%--------------------------------------------------------------------------
function [y_hat,eqn] = lognewfractionTeApply(X,mdlstruct,~)
...
eqn = " intercept + p1*sensor1 + p2*sensor2 + p3*temp + p4*hum + p5*SensInteract + p6*telapsed, where SensInteract = log((Fig2600/Fig2600+Fig2602))";

end
%--------------------------------------------------------------------------
%% Case 3 - Apply model to full colocation data
%--------------------------------------------------------------------------
function [y_hat,eqn] = lognewfractionTeApplyColo(X,fittedMdl,~)
...
%--------------------------------------------------------------------------
function [y_hat,eqn] = lognewfractionTeApplyField(X_field,fittedMdl,~)
...
%--------------------------------------------------------------------------
function lognewfractionTeReport(mdl,~)
try
    figure;
