...
switch a
    case 1; func = @ratioSens2Gen;
    case 2; func = @ratioSens2Apply;
    case 3; func = @ratioSens2ApplyColo;
    case 4; func = @ratioSens2ApplyField;
    case 5; func = @ratioSens2Report;
end
...
%--------------------------------------------------------------------------
function mdlstruct = ratioSens2Gen(Y,X,settingsSet)
...
%--------------------------------------------------------------------------
function [y_hat,eqn] = ratioSens2Apply(X,mdlstruct,~)
...
C.SensInteract = C.(extraSensor)./C.(mainSensor);

%% Simple way
y_hat = predict(mdl,C);

%get the equation of this model (used if settingsSet.database = true)
eqn = " intercept + p1*sensor1 (Fig2600) + p2*sensor2 (Fig2602) + p3*temp + p4*hum + p5*SensInteract, where SensInteract = Fig2602/Fig2600";

end
%--------------------------------------------------------------------------
%% Case 3 - Apply model to full colocation data
%--------------------------------------------------------------------------
function [y_hat,eqn] = ratioSens2ApplyColo(X,fittedMdl,~)
...
%--------------------------------------------------------------------------
function [y_hat,eqn] = ratioSens2ApplyField(X_field,fittedMdl,~)
