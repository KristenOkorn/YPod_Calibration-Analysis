function func = VOClin(a)
%Uses both VOC sensors on their own
%No VOC interaction term
%Fit the sensor response as a function of pollutant 1, pollutant 2, temperature & humidity
%Note: This function assumes that your data has columns named
%"temperature" and "humidity". It will break if they are not
%in your pod data.

switch a
    case 1; func = @VOClinGen;
    case 2; func = @VOClinApply;
    case 3; func = @VOClinApplyColo;
    case 4; func = @VOClinApplyField;
    case 5; func = @VOClinReport;
end
...
%--------------------------------------------------------------------------
function mdlstruct = VOClinGen(Y,X,settingsSet)
...
%--------------------------------------------------------------------------
function [y_hat,eqn] = VOClinApply(X,mdlstruct,~)
...
eqn = " intercept + p1*sensor1 (Fig2600) + p2*sensor2 (Fig2602) + p3*temp + p4*hum";

end
%--------------------------------------------------------------------------
%% Case 3 - Apply model to full colocation data
%--------------------------------------------------------------------------
function [y_hat,eqn] = VOClinApplyColo(X,fittedMdl,~)
...
%--------------------------------------------------------------------------
function [y_hat,eqn] = VOClinApplyField(X_field,fittedMdl,~)
