function func = lognewfraction(a)
%Fit the sensor response as a function of 2 pollutants (VOC 1 & 2), temperature, humidity, and include a VOC interaction term
%Note: This function assumes that your data has columns named
%"temperature" and "humidity".  It will break if they are not
%in your pod data.

switch a
    case 1; func = @lognewfractionGen;
    case 2; func = @lognewfractionApply;
    case 3; func = @lognewfractionApplyColo;
    case 4; func = @lognewfractionApplyField;
    case 5; func = @lognewfractionReport;
end
...
%--------------------------------------------------------------------------
function mdlstruct = lognewfractionGen(Y,X,settingsSet)
...
C.SensInteract = log(X.Fig2600./(X.Fig2600+X.Fig2602));
...
%--------------------------------------------------------------------------
function [y_hat,eqn] = lognewfractionApply(X,mdlstruct,~)
...
C.SensInteract = log(C.(mainSensor)./(C.(mainSensor)+C.(extraSensor))); %Add the temperature interaction column

%% Simple way
y_hat = predict(mdl,C);

%get the equation of this model (used if settingsSet.database = true)
eqn = " intercept + p1*sensor1 + p2*sensor2 + p3*temp + p4*hum + p5*SensInteract, where SensInteract = log((Fig2600/Fig2600+Fig2602))";
end
%--------------------------------------------------------------------------
%% Case 3 - Apply model to full colocation data
%--------------------------------------------------------------------------
function [y_hat,eqn] = lognewfractionApplyColo(X,fittedMdl,~)

%Add in the interaction term
X.SensInteract = log(X.Fig2600./(X.Fig2600+X.Fig2602)); 
...
%--------------------------------------------------------------------------
function [y_hat,eqn] = lognewfractionApplyField(X_field,fittedMdl,~)

%Add in the interaction term
X_field.SensInteract = log(X_field.Fig2600./(X_field.Fig2600+X_field.Fig2602)); 
...
%--------------------------------------------------------------------------
function lognewfractionReport(mdl,~)
try
    figure;
