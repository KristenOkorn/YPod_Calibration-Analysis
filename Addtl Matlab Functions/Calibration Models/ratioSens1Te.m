function func = ratioSens1Te(a)
%% Best model for CH4, TNMHCs as of 10/2021
...
switch a
    case 1; func = @ratioSens1TeGen;
    case 2; func = @ratioSens1TeApply;
    case 3; func = @ratioSens1TeApplyColo;
    case 4; func = @ratioSens1TeApplyField;
    case 5; func = @ratioSens1TeReport;
end
...
%--------------------------------------------------------------------------
function mdlstruct = ratioSens1TeGen(Y,X,settingsSet)
...
C.SensInteract = X.Fig2600./X.Fig2602;
...
%--------------------------------------------------------------------------
function [y_hat,eqn] = ratioSens1TeApply(X,mdlstruct,~)
...
C.SensInteract = C.(mainSensor)./C.(extraSensor); %Add the temperature interaction column

%% Simple way
y_hat = predict(mdl,C);

%get the equation of this model (used if settingsSet.database = true)
eqn = " intercept + p1*sensor1 (Fig2600) + p2*sensor2 (Fig2602) + p3*temp + p4*hum + p5*SensInteract + p6*telapsed, where SensInteract = Fig2600/Fig2602";

end
%--------------------------------------------------------------------------
%% Case 3 - Apply model to full colocation data
%--------------------------------------------------------------------------
function [y_hat,eqn] = ratioSens1TeApplyColo(X,fittedMdl,~)

%Add in the interaction term
X.SensInteract = X.Fig2600./X.Fig2602;
...
%--------------------------------------------------------------------------
function [y_hat,eqn] = ratioSens1TeApplyField(X_field,fittedMdl,~)
...
%--------------------------------------------------------------------------
function ratioSens1TeReport(mdl,~)
try
    figure;
