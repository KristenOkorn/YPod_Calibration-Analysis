...
switch a
    case 1; func = @multiSensTeGen;
    case 2; func = @multiSensTeApply;
    case 3; func = @multiSensTeApplyColo;
    case 4; func = @multiSensTeApplyField;
    case 5; func = @multiSensTeReport;
end
...
%--------------------------------------------------------------------------
function mdlstruct = multiSensTeGen(Y,X,settingsSet)
...
%--------------------------------------------------------------------------
function [y_hat,eqn] = multiSensTeApply(X,mdlstruct,~)
...
C.telapsed = X.telapsed; %Add in the elapsed time column
C.SensInteract = C.(mainSensor).*C.(extraSensor);

%% Simple way
y_hat = predict(mdl,C);

%get the equation of this model (used if settingsSet.database = true)
eqn = " intercept + p1*sensor1 + p2*sensor2 + p3*temp + p4*hum + p5*SensInteract + p6*telapsed, where SensInteract = Fig2600*Fig2602";

end
%--------------------------------------------------------------------------
%% Case 3 - Apply model to full colocation data
%--------------------------------------------------------------------------
function [y_hat,eqn] = multiSensTeApplyColo(X,fittedMdl,~)
...
%--------------------------------------------------------------------------
function [y_hat,eqn] = multiSensTeApplyField(X_field,fittedMdl,~)
...
%--------------------------------------------------------------------------
function multiSensTeReport(mdl,~)
try
    figure;
