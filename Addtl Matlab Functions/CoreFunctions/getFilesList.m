...
colo2PodDir = [analyzeDir slash 'Colocation2' slash 'Pods'];
...
%% Get secondary colocation pod data files from subfolder
fileList.colocation2.pods.dir = colo2PodDir; 
% Get the list of every raw file in the folder selected
fileList.colocation2.pods.files = dir(fullfile(fileList.colocation2.pods.dir,'*.txt')); 
%Ignore hidden files that are auto generated by OSX
fileList.colocation2.pods.files = fileList.colocation2.pods.files([fileList.colocation2.pods.files.bytes]~=4096);
%Convert the list of secondary colocated pod files to a table if it's not empty
if isempty(fileList.colocation2.pods.files)
    fileList.colocation2.pods.files=table([],[],[],[],[],[],[],[],[],[],'VariableNames',{'name','folder','date','bytes','isdir','datenum','podType','podName','hasHeaders','delimiter'});
else
    fileList.colocation2.pods.files = struct2table(fileList.colocation2.pods.files);
end
...
ncolo2Files = length(fileList.colocation2.pods.files.bytes);
...
[fileList.colocation2.pods.files.podType] = cell(ncolo2Files,1);
[fileList.colocation2.pods.files.podName] = cell(ncolo2Files,1);
[fileList.colocation2.pods.files.hasHeaders] = zeros(ncolo2Files,1);
[fileList.colocation2.pods.files.delimiter] = cell(ncolo2Files,1);
...
[fileList.colocation2.pods.files, colo2PodList] = modFileLists(fileList.colocation2.pods.files);
[fileList.field.pods.files, fieldPodList] = modFileLists(fileList.field.pods.files);

%Join Pod Lists
tempPodList = [colocPodList colo2PodList fieldPodList];
