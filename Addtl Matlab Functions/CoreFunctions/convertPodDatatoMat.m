...
%Convert colocation2 (not colocated w/ actual reference) pod files to .mat
if ~isempty(settingsSet.fileList.field.pods.files)
    disp('-Converting secondary colocation pod files...')
    readFiles(settingsSet.fileList.colocation2.pods,settingsSet.podList);
end
