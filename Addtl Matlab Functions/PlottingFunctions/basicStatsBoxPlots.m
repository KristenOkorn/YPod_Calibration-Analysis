...
plottableFuncs = {'podRMSE','podR2','podMBE'};
statnames = {'RMSE','R^2','Bias'};
...
for zz = 1:length(plottableFuncs)
        ...
        for j = 1:nVal %Calculated on each validation set
            ...
            for k = 1:nFolds %Calculated on each fold of that validation
                for z = 1:2 %Calculated for calibration data (1) or validation (2)
                    ...
                    if z==1;calval(ind)='Cal';else;calval(ind)='Val';end
    ...
    g(1,zz).stat_boxplot();
    g(1,zz).set_names('x','Model', 'y',statnames{zz}, 'color','Validation','lightness','Cal/Val');
